...
| Accuracy (Art. 5(1)(d)) | Scout's outputs are AI-generated and may contain errors. Human review is the control to catch inaccuracies before they affect candidates. Accuracy testing protocols are in development. [See `L3-4.1-Monitoring-Framework-v1.md` (forthcoming)] |
...
| **Anonymised screening** | Removing name and contact details before AI processing could reduce name-based bias risk. | [ASSUMPTION] Not currently implemented; recommend considering as a technical control in `L3-4.2-Bias-Monitoring-Protocol-v1.md` (forthcoming) |
...
| **Mitigations** | Mandatory human review (A-007); bias monitoring protocol (`L3-4.2`, forthcoming); customer training on criteria specification (`L1-2.4`); equality compliance controls (`L2-3.2`); incident response plan (`L3-4.3`, forthcoming) |
...
| **Mitigations** | Mandatory human review as operational control, not just policy; monitoring of human review compliance rate (`L3-4.1`, forthcoming); customer onboarding training; Art. 22C rights surfaced in candidate transparency notice (`L4-5.2`, forthcoming) |
...
| **Pre-mitigation risk score** | 6/9 |
| **Mitigations** | Candidate-facing transparency notice template (`L4-5.2`, forthcoming); contractual obligation on customers to deploy notice before processing (`L4-5.1`, forthcoming); audit of customer notice deployment (`L3-4.1`, forthcoming) |
...
| **Mitigations** | AWS security controls and encryption in transit/at rest [ASSUMPTION A-004]; Anthropic DPA [ASSUMPTION A-005]; incident response plan (`L3-4.3`, forthcoming); 72-hour ICO notification under Art. 33 |
...
| **Pre-mitigation risk score** | 6/9 |
| **Mitigations** | Controller/processor analysis in `L1-2.2-Risk-Classification-Framework-v1.md`; legal review of role determination; DPA templates covering both customer types (`L4-5.1`, forthcoming) |
...
| R1 — Algorithmic bias | Bias monitoring protocol with alert thresholds | CTO | Forthcoming | `L3-4.2` (forthcoming) |
| R1 — Algorithmic bias | Customer training on non-discriminatory criteria | Customer Success Lead | Forthcoming | `L1-2.4` |
| R2 — Special category data | Technical detection of special category data in inputs | Engineering Lead | Not yet implemented [ASSUMPTION] | `L3-4.2` (forthcoming) |
| R2 — Special category data | Legal advice on lawful basis route | CTO / legal counsel | [LEGAL REVIEW REQUIRED] | — |
| R3 — Unlawful ADM | Human review compliance monitoring | CTO | Forthcoming | `L3-4.1` (forthcoming) |
| R3 — Unlawful ADM | Art. 22C rights in candidate notice | Customer Success Lead | Forthcoming | `L4-5.2` (forthcoming) |
| R4 — Transparency failure | Candidate transparency notice template | Customer Success Lead | Forthcoming | `L4-5.2` (forthcoming) |
| R4 — Transparency failure | Contractual obligation on customers to deploy notice | CTO / legal counsel | Forthcoming | `L4-5.1` (forthcoming) |
| R5 — Data breach | AWS encryption and security controls | Engineering Lead | [ASSUMPTION — verify] | `L1-2.3` |
| R5 — Data breach | Incident response plan with 72h ICO notification | CTO | Forthcoming | `L3-4.3` (forthcoming) |
...
**Implementation:** `L4-5.2-Candidate-Transparency-Notice-v1.md` (forthcoming) must surface these rights. Customer contracts (`L4-5.1`, forthcoming) must impose an obligation to provide access to these rights on request.
